...
# feedforward layer: affine transformation + point-wise nonlinearity, and keeps dimensionality unchanged
...
def init_params(options):
    #dict can remember the order when added
    params = OrderedDict()
    ...
    #default lstm_encoder:false so jump
    ...
    for lidx in xrange(1, options['n_layers_init']):
        #get_layer('ff')[0] is the layer function, here is forward layer
...
def build_sampler(tparams, options, use_noise, trng, sampling=True):
    ...
    f_next: theano function
        Takes the previous word/state/memory + ctx0 and runs next
...
def train(dim_word=100,  # word vector dimensionality
    ...
    # to get the cost after regularization or the gradients, use this
